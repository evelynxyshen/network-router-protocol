...
#define DEFAULT_FILTER "filters"
#define DEFAULT_TOPO 0
#define DEFAULT_ICMP_TIMEOUT 60
#define DEFAULT_TCP_ESTAB_TIMEOUT 7440
#define DEFAULT_TCP_TRANSIT_TIMEOUT 300
...
static void sr_load_filters(struct sr_instance* sr, char* filter);
...
int main(int argc, char **argv)
{
  ...
  int tcp_estab_timeout = DEFAULT_TCP_ESTAB_TIMEOUT;
  int tcp_transit_timeout = DEFAULT_TCP_TRANSIT_TIMEOUT;
  ...
  strncpy(sr.host,host,32);
   ...
   /* -- set up filters from file -- */
  sr_load_filters(&sr, filter);
  ...
  sr.nat_enabled = nat_en;
  fprintf(stderr, "*****************INITIALIZE TIMEOUT ****************\n");
  sr.nat_icmp_timeout = icmp_timeout;
  sr.nat_tcp_estab_timeout = tcp_estab_timeout;
  sr.nat_tcp_transit_timeout = tcp_transit_timeout;
  sr_init(&sr);
...
void sr_print_filter_list(struct vns_filter *filter_list) {
  if (filter_list == NULL) {
    printf("no filters specified. accepting all connections.\n");
    return;
  }
  struct vns_filter *filter;
  for (filter = filter_list; filter != NULL; filter = filter->next) {
    printf("accepting packets ");
    printf("from %s/%d ", inet_ntoa(filter->addr), filter->mask_bits);

    if (filter->interface[0] == 0)
      printf("on all interfaces.");
    else
      printf("on %s (regardless of incoming interface).", filter->interface);
    printf("\n");
  }
}

static void sr_load_filters(struct sr_instance *sr, char *filename) {
  FILE *fp;
  int mask_bits = -1;
  int count = 0;
  int scanned = 0;
  ...
  if (access(filename, R_OK) != 0) {
    printf("Unable to access filter file: %s.\n", filename);
    return;
  }
  ...
  for (; fgets(line, BUFSIZ, fp); count++) {

    if (line[0] == '#') {
      fprintf(stderr, "Skipping commented out filter %s", line);
      continue;
    }

    filter = (struct vns_filter *)malloc(sizeof(struct vns_filter));
    //memset(filter->interface, 0, sizeof(struct vns_filter));
    memset(filter, 0, sizeof(struct vns_filter));
    assert(filter);

    scanned = sscanf(line, "%[0-9.]/%d %s", addr, &mask_bits,filter->interface);

    if (scanned != 2 && scanned != 3) {
      fprintf(stderr, "scan: %d addr: %s mask:%d\n", scanned, addr, mask_bits);
      fprintf(stderr, "Error loading filter '%s', incorrect format.\n", line);
      free(filter);
      exit(-1);
    }

    if (inet_aton(addr, &(filter->addr)) == 0) {
      fprintf(stderr, "Error loading filters, cannot convert %s to valid IP\n",
        addr);
      free(filter);
      exit(-1);
    }
    ...
    if (mask_bits < 0 || mask_bits > 32) {
      fprintf(stderr, "Error loading filters, mask specified (%d) not in valid "
        "range (0-32).\n", mask_bits);
      exit(-1);
    }
    filter->mask_bits = mask_bits;
    filter->mask.s_addr = ntohl(~(0xffffffffL >> mask_bits));

    filter->next = sr->filter_list;
    sr->filter_list = filter;
  }

  printf("\nLoading filters\n");
  printf("---------------------------------------------\n");
  sr_print_filter_list(sr->filter_list);
  printf("---------------------------------------------\n");
}
