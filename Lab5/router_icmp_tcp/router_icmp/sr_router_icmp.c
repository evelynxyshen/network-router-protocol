        ...
        char* interface/* lent */)
{
  ...
  if(sr->nat_enabled){
	...
	sr_handlepacket_forwarding(sr, packet, len, interface, 0);
	/*// need to get the mac of internal node
       	  // lookup the internal node in the arp cache
	  struct sr_arpentry* arp_entry = sr_arpcache_lookup(&sr->cache, entry->ip_int);
	  struct sr_arpreq* arp_req = 0;
  	  /* if the nexthop_ip is found in the arp cache
  	     use it directly to generate the reply packet */
  	  /*if(arp_entry){
		fprintf(stderr, "ARP Found. \n");
		memcpy(ehdr->ether_dhost, arp_entry->mac, 6);
		sr_send_packet(sr, packet, len, interface);
	  }else{
		arp_req = sr_arpcache_queuereq(&sr->cache, entry->ip_int, packet, len, interface);
