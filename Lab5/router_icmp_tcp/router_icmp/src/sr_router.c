        ...
        char* interface/* lent */)
{
  ...
  if(sr->nat_enabled){
    if(tcphdr->tcp_syn){
      // hold the packet for 6 sec, then check if connection is initiated from internal ip
    }
    ...
    if(entry){
      ...
      if((has_connect->state == nat_connection_building)&&(tcphdr->tcp_ack)){
...
void sr_handle_forwardtcp_nat(struct sr_instance* sr,
  ...
  if(strncmp(interface, "eth0", 4) == 0){
    ...
    if(tcphdr->tcp_syn){
      ...
      entry->conns->state = nat_connection_established;
      tcphdr->tcp_src = htons(entry->aux_ext);
