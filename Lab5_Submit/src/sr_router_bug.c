        ...
        char* interface/* lent */)
{
  ...
  if(ehdr->ether_type == htons(ethertype_ip)) { //IP
	  ...
	  if(sr->nat_enabled){
	  	sr_handle_forwarding_nat(sr, sr->routing_nat, packet, len, interface);
	  }else{
...
struct sr_ethernet_hdr* create_eth_hdr(struct sr_ethernet_hdr* ehdr, uint8_t* shost, uint8_t* dhost, uint16_t type){
  ...
  if(dhost){
    memcpy(ehdr->ether_dhost, (uint8_t*)dhost, ETHER_ADDR_LEN*sizeof(uint8_t));
  }
  if(shost){
    memcpy(ehdr->ether_shost, (uint8_t*)shost, ETHER_ADDR_LEN*sizeof(uint8_t));
  }
  if(type){
...
struct sr_ip_hdr* create_ip_hdr(struct sr_ip_hdr* iphdr, uint32_t ip_src, uint32_t ip_dst, uint8_t ip_p, uint8_t ip_ttl){
  if(iphdr == NULL){
    iphdr = (struct sr_ip_hdr*) malloc(sizeof(struct sr_ip_hdr));
  }
  iphdr->ip_src = ip_src;
  iphdr->ip_dst = ip_dst;
  iphdr->ip_p = ip_p;
...
struct sr_arp_hdr* create_arp_hdr(struct sr_arp_hdr* ahdr, uint16_t ar_op, uint8_t* ar_sha, uint32_t ar_sip, uint8_t* ar_tha, uint32_t ar_tip, uint16_t ar_hrd, uint16_t ar_pro, uint8_t ar_hln, uint8_t ar_pln){
  if(ahdr == NULL){
    ahdr = (struct sr_arp_hdr*) malloc(sizeof(struct sr_arp_hdr));
  }
  ahdr->ar_op = ar_op;
  if (ar_sha){
    memcpy(ahdr->ar_sha, ar_sha, 6);
  }
  ahdr->ar_sip =  ar_sip ? ar_sip : ahdr->ar_sip;
  if (ar_tha){
    memcpy(ahdr->ar_tha, ar_tha, ETHER_ADDR_LEN * sizeof(uint8_t));
  }
  ahdr->ar_tip =  ar_tip ? ar_tip : ahdr->ar_tip;
  ahdr->ar_hrd = ar_hrd ? ar_hrd : ahdr->ar_hrd;
  ahdr ->ar_pro = ar_pro ? ar_pro : ahdr->ar_pro;
  ahdr ->ar_hln = ar_hln ? ar_hln : 6;
  ahdr ->ar_pln = ar_pln ? ar_pln : 4;
  return ahdr;
}
...
struct sr_icmp_hdr* create_icmp_hdr(struct sr_icmp_hdr* icmphdr, uint8_t type, uint8_t code){
  int icmp_len = sizeof(struct sr_icmp_hdr);
  if(icmphdr == NULL){
    icmphdr = (struct sr_icmp_hdr *)malloc(icmp_len);
  }
...
void sr_handle_forwarding_nat(struct sr_instance* sr,
	struct sr_nat* nat, 
	uint8_t * packet/* lent */,
        unsigned int len,
        char* interface/* lent */)
