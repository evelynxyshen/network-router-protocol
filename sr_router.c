        ...
        uint8_t code)
{
  ...
  /* The reply packet has ethernet hdr, ip hdr, icmp hdr, 4B reserved space, old ip hdr and 8B ip payload */ 
  uint8_t* reply_pkt = 0;
  struct sr_ethernet_hdr* reply_ehdr = 0;
  struct sr_ip_hdr* reply_iphdr = 0;
  ...
  // print_hdr_icmp ((uint8_t*)reply_icmphdr);
  // print_hdr_icmp ((uint8_t*)reply_icmphdr_sec);
  memcpy(reply_pkt+sizeof(sr_ethernet_hdr_t)+sizeof(sr_ip_hdr_t), (uint8_t*) reply_icmphdr, icmp_len);
