...
void
check_destroy (rel_t *r)
{
  if ((r->eof_acked)&&(r->eof_recved)&&(r->wait_ack == 0)&&(r->all_output)){
    fprintf(stderr, "NOTE: connection destroyed.\n");
    rel_destroy(r);
  }else
    return;
}
      ...
      if (conn_bufspace(r->c)>=data_len){
        ...
        if (r->outbuffer_full==0){
      	...
      	if (r->outbuffer_full)
	        ...
	        memcpy(r->outbuffer_data, pkt->data, sizeof(char)*data_len);
	        r->outbuffer_data_len = data_len;
	        r->outbuffer_full = 1;
          r->all_output = 0;
	      }
      }
...
rel_output (rel_t *r)
{
  if (conn_bufspace(r->c) < r->outbuffer_data_len)
    return;
  else {
    conn_output (r->c, r->outbuffer_data, r->outbuffer_data_len);
    r->all_output = 1;
